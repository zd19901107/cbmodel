/**
  ...
  *
  *This program uses the library CbModel for local video, it allows you to view the video test 
  *and the results obtained from CbModel on the video test. 
  ...
  if(frameDisplayed >= 0){
    if(frameDisplayed <= frames){  
      if(myMovie.available()){             
        ...
        if(first){
          first=false;
          Model = new CbModel(this,myMovie.width,myMovie.height, alfa, beta, e1, e2, tm, 0, 0xFFFFFFFF);
        }  
        image(myMovie, 10, 50,384,288);
        fill(255); 
      ...
      if( Model.isTestState()){
        ...
        if(myMovie.available()){
          ...
          ellipse(380, 27, 20, 20); 
          try{
	    image(Model.getDifferenceImage(myMovie), 456, 50,384,288);
          }
          catch(WrongSizeException Err){
            println("Dimension of input's image not equal to the size set in the model...");
          }
...
void mouseClicked(){
  ...
  if( mouseX_play == true && mouseY_play == true ){
    if(isPlaying == true){
      ...
      frameDisplayed = 0;    
      if( i == 0)
        myMovie = new GSMovie(this, "video/" + path); 
      else{
